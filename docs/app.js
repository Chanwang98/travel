...
async function fetchLocationWeather(location,range){
  const today=new Date();today.setHours(0,0,0,0);const start=new Date(`${range.start}T00:00:00`),end=new Date(`${range.end}T00:00:00`);const max=new Date(today);max.setDate(max.getDate()+15);const min=new Date(today);min.setDate(min.getDate()-92);
  if(end<min||start>max)return {location,status:start>max?"future":"past",days:[]};
  const toLocalDate=value=>`${value.getFullYear()}-${String(value.getMonth()+1).padStart(2,"0")}-${String(value.getDate()).padStart(2,"0")}`;
  const queryStart=start<min?toLocalDate(min):range.start,queryEnd=end>max?toLocalDate(max):range.end;
...
function renderWeather(){
  ...
  const first=results.find(x=>x.days.length),today=new Date(),todayKey=`${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,"0")}-${String(today.getDate()).padStart(2,"0")}`,upcomingDays=(first?.days||[]).filter(day=>day.date>=todayKey),firstDay=upcomingDays[0];
  $("weatherCardIcon").textContent=firstDay?weatherMeta(firstDay.code).icon:"☁︎";$("weatherCardTitle").textContent=results.map(x=>x.location.name).join("、");$("weatherCardSummary").textContent=firstDay?`${weatherMeta(firstDay.code).label} · ${firstDay.min}°–${firstDay.max}° · 降雨 ${firstDay.rain??0}%`:(results.some(x=>x.status==="future")?"暂未进入 16 天预报范围":"本次行程已结束 · 点击查看历史天气");
  $("weatherCardDays").innerHTML=upcomingDays.slice(0,4).map(day=>`<span><small>${Number(day.date.slice(5,7))}/${Number(day.date.slice(8))}</small><b>${weatherMeta(day.code).icon}</b><em>${day.max}°</em></span>`).join("");
